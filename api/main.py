...
async def addAircraft(request: str = Form(...), internos: List[UploadFile] = File(...), externos: List[UploadFile] = File(...)):
    req = json.loads(request)
    req["photos_path"] = await saveImages(req, externos, internos)
